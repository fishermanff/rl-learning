...
class BanditRun(object):
    ...
    def run(self):
        ...
        for bd in bandits:
            bd.run()
        actionAndReward = [bd.actionAndReward for bd in bandits]
        for ar in actionAndReward:
            self.action.append([x[0] for x in ar])
            self.reward.append([x[1] for x in ar])
            self.isCorrectAction.append([1 if action[i]==])

    def getAverageReward(self):
        return np.array(self.reward).mean(axis=0)

    def getCorrectPercent(self):
        self.isCorrectAction = [1 if ]
